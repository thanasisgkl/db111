...
def process_articles():
    try:
        ...
        with open('141_1991-Αρμοδιότητες-οργάνων-και-υπηρεσιακές-ενέργειες-του.txt', 'r', encoding='utf-8') as file:
            lines = file.readlines()
        
        # Κρατάμε μόνο τις γραμμές 49-6707
        article_lines = lines[48:6707]  # -1 γιατί οι γραμμές ξεκινούν από 0
        ...
        # Αποθηκεύουμε σε CSV
        df.to_csv('astynomikos.csv', index=False, encoding='utf-8')
        ...
        try:
            ...
            cursor.execute("SELECT id FROM laws WHERE name = 'ΑΣΤΥΝΟΜΙΚΟΣ ΝΟΜΟΣ'")
            law_id = cursor.fetchone()
            
            if not law_id:
                # Αν δεν υπάρχει ο νόμος, τον προσθέτουμε
                cursor.execute(
                    "INSERT INTO laws (name) VALUES ('ΑΣΤΥΝΟΜΙΚΟΣ ΝΟΜΟΣ') RETURNING id"
                )
