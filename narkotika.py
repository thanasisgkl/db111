...
def process_articles():
    try:
        ...
        with open('Κώδικας-Νόμων-για-τα-Ναρκωτικά.txt', 'r', encoding='utf-8') as file:
            text = file.read()
        ...
        # Αποθηκεύουμε σε CSV
        df.to_csv('narkotika.csv', index=False, encoding='utf-8')
        ...
        try:
            ...
            cursor.execute("SELECT id FROM laws WHERE name = 'ΚΩΔΙΚΑΣ ΝΟΜΩΝ ΓΙΑ ΤΑ ΝΑΡΚΩΤΙΚΑ'")
            law_id = cursor.fetchone()
            
            if not law_id:
                # Αν δεν υπάρχει ο νόμος, τον προσθέτουμε
                cursor.execute(
                    "INSERT INTO laws (name) VALUES ('ΚΩΔΙΚΑΣ ΝΟΜΩΝ ΓΙΑ ΤΑ ΝΑΡΚΩΤΙΚΑ') RETURNING id"
                )
