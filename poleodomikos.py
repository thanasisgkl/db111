...
def process_articles():
    try:
        ...
        with open('Κώδικας-Βασικής-Πολεοδομικής-Νομοθεσίας.txt', 'r', encoding='utf-8') as file:
            lines = file.readlines()
        
        # Κρατάμε μόνο τις γραμμές 12-22221
        article_lines = lines[11:22221]  # -1 γιατί οι γραμμές ξεκινούν από 0
        ...
        # Αποθηκεύουμε σε CSV
        df.to_csv('poleodomikos.csv', index=False, encoding='utf-8')
        ...
        try:
            ...
            cursor.execute("SELECT id FROM laws WHERE name = 'ΚΩΔΙΚΑΣ ΒΑΣΙΚΗΣ ΠΟΛΕΟΔΟΜΙΚΗΣ ΝΟΜΟΘΕΣΙΑΣ'")
            law_id = cursor.fetchone()
            
            if not law_id:
                # Αν δεν υπάρχει ο νόμος, τον προσθέτουμε
                cursor.execute(
                    "INSERT INTO laws (name) VALUES ('ΚΩΔΙΚΑΣ ΒΑΣΙΚΗΣ ΠΟΛΕΟΔΟΜΙΚΗΣ ΝΟΜΟΘΕΣΙΑΣ') RETURNING id"
                )
