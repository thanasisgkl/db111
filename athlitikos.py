...
def process_articles():
    try:
        ...
        with open('ATHLITIKOS NOMOS.txt', 'r', encoding='utf-8') as file:
            lines = file.readlines()
        
        # Κρατάμε μόνο τις γραμμές 36-9187
        article_lines = lines[35:9187]  # -1 γιατί οι γραμμές ξεκινούν από 0
        ...
        # Αποθηκεύουμε σε CSV
        df.to_csv('athlitikos.csv', index=False, encoding='utf-8')
        ...
        try:
            ...
            cursor.execute("SELECT id FROM laws WHERE name = 'ΑΘΛΗΤΙΚΟΣ ΝΟΜΟΣ'")
            law_id = cursor.fetchone()
            
            if not law_id:
                # Αν δεν υπάρχει ο νόμος, τον προσθέτουμε
                cursor.execute(
                    "INSERT INTO laws (name) VALUES ('ΑΘΛΗΤΙΚΟΣ ΝΟΜΟΣ') RETURNING id"
                )
