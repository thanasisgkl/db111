...
def process_articles():
    try:
        ...
        with open('Κώδικας-Δημόσιου-Ναυτικού-Δικαίου.txt', 'r', encoding='utf-8') as file:
            lines = file.readlines()
        
        # Κρατάμε μόνο τις γραμμές 22-6177
        article_lines = lines[21:6177]  # -1 γιατί οι γραμμές ξεκινούν από 0
        ...
        # Αποθηκεύουμε σε CSV
        df.to_csv('naytiko.csv', index=False, encoding='utf-8')
        ...
        try:
            ...
            cursor.execute("SELECT id FROM laws WHERE name = 'ΚΩΔΙΚΑΣ ΔΗΜΟΣΙΟΥ ΝΑΥΤΙΚΟΥ ΔΙΚΑΙΟΥ'")
            law_id = cursor.fetchone()
            
            if not law_id:
                # Αν δεν υπάρχει ο νόμος, τον προσθέτουμε
                cursor.execute(
                    "INSERT INTO laws (name) VALUES ('ΚΩΔΙΚΑΣ ΔΗΜΟΣΙΟΥ ΝΑΥΤΙΚΟΥ ΔΙΚΑΙΟΥ') RETURNING id"
                )
