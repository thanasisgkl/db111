...
def process_articles():
    try:
        ...
        with open('Κώδικας-Φορολογικής-Δικονομίας.txt', 'r', encoding='utf-8') as file:
            lines = file.readlines()
        
        # Κρατάμε μόνο τις γραμμές 21-3645
        article_lines = lines[20:3645]  # -1 γιατί οι γραμμές ξεκινούν από 0
        ...
        # Αποθηκεύουμε σε CSV
        df.to_csv('forologiki_dikonomia.csv', index=False, encoding='utf-8')
        ...
        try:
            ...
            cursor.execute("SELECT id FROM laws WHERE name = 'ΚΩΔΙΚΑΣ ΦΟΡΟΛΟΓΙΚΗΣ ΔΙΚΟΝΟΜΙΑΣ'")
            law_id = cursor.fetchone()
            
            if not law_id:
                # Αν δεν υπάρχει ο νόμος, τον προσθέτουμε
                cursor.execute(
                    "INSERT INTO laws (name) VALUES ('ΚΩΔΙΚΑΣ ΦΟΡΟΛΟΓΙΚΗΣ ΔΙΚΟΝΟΜΙΑΣ') RETURNING id"
                )
