...
def process_articles():
    try:
        ...
        with open('Κώδικας-Αεροπορικού-Δικαίου.txt', 'r', encoding='utf-8') as file:
            lines = file.readlines()
        
        # Κρατάμε μόνο τις γραμμές 27-3203
        article_lines = lines[26:3203]  # -1 γιατί οι γραμμές ξεκινούν από 0
        ...
        # Αποθηκεύουμε σε CSV
        df.to_csv('aeroporiko.csv', index=False, encoding='utf-8')
        ...
        try:
            ...
            cursor.execute("SELECT id FROM laws WHERE name = 'ΚΩΔΙΚΑΣ ΑΕΡΟΠΟΡΙΚΟΥ ΔΙΚΑΙΟΥ'")
            law_id = cursor.fetchone()
            
            if not law_id:
                # Αν δεν υπάρχει ο νόμος, τον προσθέτουμε
                cursor.execute(
                    "INSERT INTO laws (name) VALUES ('ΚΩΔΙΚΑΣ ΑΕΡΟΠΟΡΙΚΟΥ ΔΙΚΑΙΟΥ') RETURNING id"
                )
