...
def process_articles():
    try:
        ...
        with open('Υπαλληλικός-Κώδικας.txt', 'r', encoding='utf-8') as file:
            lines = file.readlines()
        
        # Κρατάμε μόνο τις γραμμές 24-5338
        article_lines = lines[23:5338]  # -1 γιατί οι γραμμές ξεκινούν από 0
        ...
        # Αποθηκεύουμε σε CSV
        df.to_csv('ypalilikos.csv', index=False, encoding='utf-8')
        ...
        try:
            ...
            cursor.execute("SELECT id FROM laws WHERE name = 'ΥΠΑΛΛΗΛΙΚΟΣ ΚΩΔΙΚΑΣ'")
            law_id = cursor.fetchone()
            
            if not law_id:
                # Αν δεν υπάρχει ο νόμος, τον προσθέτουμε
                cursor.execute(
                    "INSERT INTO laws (name) VALUES ('ΥΠΑΛΛΗΛΙΚΟΣ ΚΩΔΙΚΑΣ') RETURNING id"
                )
