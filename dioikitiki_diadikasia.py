...
def process_articles():
    try:
        ...
        with open('Κώδικας-Διοικητικής-Διαδικασίας-_1_.txt', 'r', encoding='utf-8') as file:
            lines = file.readlines()
        
        # Κρατάμε μόνο τις γραμμές 22-1209
        article_lines = lines[21:1209]  # -1 γιατί οι γραμμές ξεκινούν από 0
        ...
        # Αποθηκεύουμε σε CSV
        df.to_csv('dioikitiki_diadikasia.csv', index=False, encoding='utf-8')
        ...
        try:
            ...
            cursor.execute("SELECT id FROM laws WHERE name = 'ΚΩΔΙΚΑΣ ΔΙΟΙΚΗΤΙΚΗΣ ΔΙΑΔΙΚΑΣΙΑΣ'")
            law_id = cursor.fetchone()
            
            if not law_id:
                # Αν δεν υπάρχει ο νόμος, τον προσθέτουμε
                cursor.execute(
                    "INSERT INTO laws (name) VALUES ('ΚΩΔΙΚΑΣ ΔΙΟΙΚΗΤΙΚΗΣ ΔΙΑΔΙΚΑΣΙΑΣ') RETURNING id"
                )
