...
def process_articles():
    try:
        ...
        with open('EMPORIKOS-NOMOS.txt', 'r', encoding='utf-8') as file:
            text = file.read()
        ...
        pattern = r'Αρθρο:\s*(\d+).*?(?:Κείμενο\s*Αρθρου\s*(.*?)(?=(?:\s*Αρθρο:|$)))'
        ...
        for article in articles:
            ...
            content = article[1].strip() if article[1] else None
            
            # Χρησιμοποιούμε την αρχή του περιεχομένου ως τίτλο
        ...
        # Αποθηκεύουμε σε CSV
        df.to_csv('emporikos.csv', index=False, encoding='utf-8')
        ...
        try:
            ...
            cursor.execute("SELECT id FROM laws WHERE name = 'ΕΜΠΟΡΙΚΟΣ ΝΟΜΟΣ'")
            law_id = cursor.fetchone()
            
            if not law_id:
                # Αν δεν υπάρχει ο νόμος, τον προσθέτουμε
                cursor.execute(
                    "INSERT INTO laws (name) VALUES ('ΕΜΠΟΡΙΚΟΣ ΝΟΜΟΣ') RETURNING id"
                )
