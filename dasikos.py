...
def process_articles():
    try:
        ...
        with open('Δασικός-Κώδικας.txt', 'r', encoding='utf-8') as file:
            lines = file.readlines()
        
        # Κρατάμε μόνο τις γραμμές 21-9649
        article_lines = lines[20:9649]  # -1 γιατί οι γραμμές ξεκινούν από 0
        ...
        # Αποθηκεύουμε σε CSV
        df.to_csv('dasikos.csv', index=False, encoding='utf-8')
        ...
        try:
            ...
            cursor.execute("SELECT id FROM laws WHERE name = 'ΔΑΣΙΚΟΣ ΚΩΔΙΚΑΣ'")
            law_id = cursor.fetchone()
            
            if not law_id:
                # Αν δεν υπάρχει ο νόμος, τον προσθέτουμε
                cursor.execute(
                    "INSERT INTO laws (name) VALUES ('ΔΑΣΙΚΟΣ ΚΩΔΙΚΑΣ') RETURNING id"
                )
