...
def process_articles():
    try:
        ...
        with open('Κώδικας-Εισπράξεως-Δημοσίων-Εσόδων.txt', 'r', encoding='utf-8') as file:
            text = file.read()
        ...
        # Αποθηκεύουμε σε CSV
        df.to_csv('kede.csv', index=False, encoding='utf-8')
        ...
        try:
            ...
            cursor.execute("SELECT id FROM laws WHERE name = 'ΚΩΔΙΚΑΣ ΕΙΣΠΡΑΞΕΩΣ ΔΗΜΟΣΙΩΝ ΕΣΟΔΩΝ'")
            law_id = cursor.fetchone()
            
            if not law_id:
                # Αν δεν υπάρχει ο νόμος, τον προσθέτουμε
                cursor.execute(
                    "INSERT INTO laws (name) VALUES ('ΚΩΔΙΚΑΣ ΕΙΣΠΡΑΞΕΩΣ ΔΗΜΟΣΙΩΝ ΕΣΟΔΩΝ') RETURNING id"
                )
